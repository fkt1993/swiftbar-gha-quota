...
macOS のステータスバーに `GHA █████░░░ 62%` のように今月の GitHub Actions 無料枠の消化率を
バーで出すプラグイン。
...
バーは 8 セル固定幅で、1 セルあたり 12.5%。100% を超えても満杯で止まる（色で気づく）。
...
| `label` | ステータスバーの接頭辞。`""` にすればバーだけ |
| `bar_style` | バーの描き方。下表参照。`"none"` でバーを消して数字だけ |
| `bar_cells` | バーの桁数。既定 8（1 セル = 12.5%） |
| `show_percent` | `false` にすると `%` を消してバーだけ |
...
| 値 | 62% のとき | 備考 |
| --- | --- | --- |
| `solid` | `█████░░░` | 既定。コントラストが強く一目で読める |
| `shade` | `▓▓▓▓▓▒▒▒` | 柔らかめ。メニューバーに馴染むが境目が分かりにくい |
| `dots` | `●●●●●○○○` | 丸ドット |
| `eighth` | `████▉░░░` | 1/8 ブロックで滑らかに伸びる。分解能は 8 倍 |
| `none` | （なし） | バーを出さず `GHA 62%` に戻す |

バーは等幅で描かないと桁が揃わないので `font=Menlo` を指定している。
