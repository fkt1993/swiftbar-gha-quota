...
**メニューバーはバー表記。** `bar()` がブロック文字（`█`/`░`）で 8 セルのバーを描き、
`title_for()` が「ラベル + バー + %」を組む。空の要素は詰めるので、`label=""` や
`show_percent=false` にしても余計な空白が残らない。等幅でないと桁が揃わないので
タイトル行の `font=Menlo` は外さないこと。`bar_style` / `bar_cells` / `show_percent` で config から変えられる。
